  ...
  try {
    ...
    if (seat_id) {
      ...
      if (isNaN(seatIdNumber)) {
        return new Response(JSON.stringify({ message: "Invalid seat ID" }), {
          status: 400,
        });
      }
      const [seat] = await pool.query("SELECT * FROM seat WHERE seat_id = ?", [
        seatIdNumber,
      ]);
      if (seat.length === 0) {
    ...
    return new Response(
      JSON.stringify({ message: "Error fetching seat", error: error.message }),
      { status: 500 }
    );
  }
}
...
export async function POST(req) {
  try {
    ...
    return new Response(
      JSON.stringify({ message: "Error adding seat", error: error.message }),
      { status: 500 }
    );
  }
}
...
export async function PUT(req) {
  try {
    ...
    return new Response(
      JSON.stringify({ message: "Error updating seat", error: error.message }),
      { status: 500 }
    );
  }
}
...
export async function DELETE(req) {
  try {
    ...
    const [result] = await pool.query("DELETE FROM seat WHERE seat_id = ?", [
      Number(seat_id),
    ]);
    if (result.affectedRows === 0) {
      return new Response(JSON.stringify({ message: "Seat not found" }), {
    ...
    return new Response(
      JSON.stringify({ message: "Error deleting seat", error: error.message }),
      { status: 500 }
    );
  }
}
